...
class DanTangViewController: GFBaseViewController, UIScrollViewDelegate{

    var channels = [GFChannel]()
    //标签
    weak var titlesView = UIView.init()
    //底部红色指示器
    weak var indicatorView = UIView.init()
    
    weak var contentView = UIScrollView.init()
    ...
    override func viewDidLoad() {
        ...
        // 获取首页顶部选择数据
        GFNetworkTool.shareNetworkTool.loadHomeTopData { (ym_channels) in
    ...
    func setupTitlesView() {
        ...
        bgView.frame = CGRect(x: 0, y: kTitlesViewY, width: KMainWidth, height: kTitlesViewH)
        view.addSubview(bgView)
        // 标签
        let titlesView = UIView.init()
        titlesView.backgroundColor = GFGlobalColor()
        titlesView.frame = CGRect(x: 0, y: 0, width: KMainWidth - kTitlesViewH, height: kTitlesViewH)
        bgView.addSubview(titlesView)
        self.titlesView = titlesView
        //底部红色指示器
        let indicatorView = UIView()
        indicatorView.backgroundColor = GFGlobalRedColor()
        indicatorView.frame.size.height = kIndicatorViewH
        ...
        arrowButton.frame = CGRect(x: KMainWidth - kTitlesViewH, y: 0, width: kTitlesViewH, height: kTitlesViewH)
        ...
        arrowButton.backgroundColor = GFGlobalColor()
        ...
        for index in 0..<count {
            ...
            button.setTitleColor(GFGlobalRedColor(), for: .disabled)
            ...
            if index == 0 {
                ...
                indicatorView.frame.size.width = button.titleLabel!.width
    ...
    @objc func titlesClick(button: UIButton) {
        ...
        // 让标签执行动画
        UIView.animate(withDuration: kAnimationDuration) {
            self.indicatorView!.frame.size.width = self.selectedButton!.titleLabel!.width
            self.indicatorView!.center.x = self.selectedButton!.centerX
        }
        //滚动,切换子控制器
